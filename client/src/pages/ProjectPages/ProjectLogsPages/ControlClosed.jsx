    ...
    return (
        <div className="py-10">
            <table className="w-full">
                ...
                <tbody>
                    {controlsClosedData.map((controlClosedData, index) => {
                        return (
                            <tr onClick={() => navigate(`/projects/logs/control/closed/${"45121245112"}/view-control/${'45642'}`)} className="bg-white border-b-[20px] border-light-gray cursor-pointer" key={index}>
                                <td className="py-5 px-4 text-start">{controlClosedData.control_name}</td>
                                <td className="py-5 px-4 text-start">{controlClosedData.control_owner} </td>
                                <td className="py-5 px-4 text-start">{controlClosedData.control_status.slice(20)}...</td>
                                <td className="py-5 px-4 text-start">
                                    <p className="bg-[#BBF7D0] w-fit px-3 py-[1px] text-[#158F9C] rounded-full">{controlClosedData.control_date}</p>
                                </td>
                                <td className="py-5 px-4 text-start">
                                    <p className="bg-[#BBF7D0] w-fit px-3 py-[1px] text-[#158F9C] rounded-full">{controlClosedData.control_due_date}</p>
                                </td>
                                <td className="py-5 px-4 text-start">{controlClosedData.tags}</td>
                                <td className="py-5 px-4 text-start">
                                    {controlClosedData.impact}
                                </td>
                            </tr>
                        );
                    })}
