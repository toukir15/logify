...
const Date = ({ dateData }) => {
    ...
    return (
        <div>
            <div onClick={() => setIsControlDateOpen(true)} className=' flex items-center justify-between border border-primary rounded p-3 bg-white text-[#4E81CD] cursor-pointer'>
                ...
                <div data-aos="zoom-in" onClick={(e) => e.stopPropagation()} className='bg-white p-6 rounded-lg w-[350px] min-h-[470px]'>
                    <div>
                        <div className='flex justify-end items-center text-primary '>
                            <IoMdClose onClick={() => setIsControlDateOpen(false)} className='cursor-pointer hover:text-[#293061]' size={24} />
                        </div>
